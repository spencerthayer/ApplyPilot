...
### `resume.json`
Canonical resume artifact. ApplyPilot stores `~/.applypilot/resume.json` as JSON Resume plus local `meta.applypilot` extensions for rendering, import/export, and resume-derived metadata.

### `profile.json`
Authoritative ApplyPilot profile/settings store. Runtime profile reads come from `~/.applypilot/profile.json`.

If `profile.json` is missing but `resume.json` is valid, ApplyPilot auto-generates `profile.json` once for compatibility and then continues using `profile.json` as the source of truth.
