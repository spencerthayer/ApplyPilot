...
### `resume.json`
Canonical user artifact. ApplyPilot prefers `~/.applypilot/resume.json` and validates it as JSON Resume plus local `meta.applypilot` extensions for work authorization, compensation, availability, tailoring config, resume facts, and optional files.

If `resume.json` is absent, ApplyPilot still falls back to legacy `profile.json + resume.txt`.

### `profile.json` and `resume.txt` (legacy fallback)
Still supported when `resume.json` is missing, but no longer the preferred source of truth.
