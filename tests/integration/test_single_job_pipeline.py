...
def test_db(tmp_path, monkeypatch):
    """Create an in-memory DB with the jobs table and patch get_connection."""
...
def _insert_job(conn, url="https://example.com/job/1", title="Test Job",
                site="example", desc=None, score=None, tailored=None,
                cover=None, apply_status=None, app_url=None):
...
class TestGetJobsByStageScoping:
    ...
    def test_without_job_url_returns_all(self, test_db):
        ...
        jobs = get_jobs_by_stage(conn=test_db, stage="enriched", limit=100)
        assert len(jobs) == 2
    ...
    def test_job_url_no_match_returns_empty(self, test_db):
        ...
        jobs = get_jobs_by_stage(conn=test_db, stage="enriched", limit=100,
                                 job_url="https://nonexistent.com/999")
        assert len(jobs) == 0

    def test_pending_cover_stage(self, test_db):
        from applypilot.database import get_jobs_by_stage
        _insert_job(test_db, "https://a.com/1", "Job A", desc="desc", score=8,
                    tailored="/path/a.txt")
        _insert_job(test_db, "https://b.com/2", "Job B", desc="desc", score=9,
                    tailored="/path/b.txt")

        jobs = get_jobs_by_stage(conn=test_db, stage="pending_cover",
                                 min_score=7, limit=100,
                                 job_url="https://a.com/1")
        assert len(jobs) == 1
        assert jobs[0]["url"] == "https://a.com/1"
...
class TestBulletCountConfig:
    """Verify tailoring prompt reads bullet config from profile."""
    ...
    def test_prompt_contains_bullet_range(self):
        ...
        assert "4-6 bullets per role" in prompt
        assert "aim for 6" in prompt
    ...
    def test_default_bullets_when_no_config(self):
        ...
        prompt = _build_tailor_prompt(profile)
        assert "3-5 bullets per role" in prompt
...
class TestManualAtsParking:
    ...
    def test_manual_ats_parked_in_queue_mode(self, test_db, monkeypatch):
        ...
        monkeypatch.setattr(launcher, "get_connection", lambda: test_db)
        monkeypatch.setattr(launcher, "_load_blocked", lambda: (set(), []))
        _insert_job(test_db, "https://company.myworkdayjobs.com/job/123",
                    "SWE III", site="workday", desc="desc", score=8,
                    tailored="/path.txt", app_url="https://company.myworkdayjobs.com/job/123/apply")
        _insert_job(test_db, "https://good.com/456",
                    "SWE", site="linkedin", desc="desc", score=7,
                    tailored="/path2.txt", app_url="https://good.com/456/apply")

        monkeypatch.setattr("applypilot.config.is_manual_ats",
                            lambda url: "myworkdayjobs" in url)
        # Queue mode: should park manual ATS and return the next actionable job
        result = launcher.acquire_job(min_score=7)

        row = test_db.execute("SELECT apply_status, needs_human_reason FROM jobs WHERE url LIKE '%myworkday%'").fetchone()
    ...
    def test_target_url_bypasses_manual_ats_check(self, test_db, monkeypatch):
        ...
        monkeypatch.setattr(launcher, "get_connection", lambda: test_db)
        _insert_job(test_db, "https://company.myworkdayjobs.com/job/123",
                    "SWE", site="workday", desc="desc", score=8,
                    tailored="/path.txt", app_url="https://company.myworkdayjobs.com/job/123/apply")
...
class TestHitlDbOps:
    ...
    def test_needs_human_roundtrip(self, test_db):
        ...
        test_db.execute(
            "UPDATE jobs SET apply_status = NULL, needs_human_reason = NULL WHERE url = ?",
            ("https://example.com/1",))
        test_db.commit()
        row = test_db.execute("SELECT apply_status, needs_human_reason FROM jobs").fetchone()
...
class TestScoringE2EScoping:
    """Verify run_scoring only scores the target job when job_url is set."""

    def test_scoring_scoped_to_single_url(self, test_db, monkeypatch):
        from applypilot.scoring import scorer

        _insert_job(test_db, "https://target.com/1", "Target SWE", desc="Python engineer needed")
        _insert_job(test_db, "https://other.com/2", "Other Job", desc="Sales manager role")

        monkeypatch.setattr(scorer, "get_connection", lambda: test_db)
        monkeypatch.setattr(scorer, "_load_scoring_resume_text", lambda: "Python developer with 3 years exp")
        monkeypatch.setattr(scorer, "score_job", lambda *a, **kw: {
            "score": 8, "keywords": "python", "reasoning": "good fit",
            "matched_skills": ["python"], "missing_requirements": [],
            "baseline_score": 7, "llm_score": 8, "llm_confidence": 0.9,
            "score_delta": 1, "normalized_title": "target swe",
        })

        result = scorer.run_scoring(job_url="https://target.com/1")

        # Only target job scored
        target = test_db.execute("SELECT fit_score FROM jobs WHERE url = 'https://target.com/1'").fetchone()
        other = test_db.execute("SELECT fit_score FROM jobs WHERE url = 'https://other.com/2'").fetchone()
        assert target["fit_score"] is not None
        assert other["fit_score"] is None

    def test_scoring_without_url_scores_all(self, test_db, monkeypatch):
        from applypilot.scoring import scorer
        ...
        def mock_score(*a, **kw):
            call_count["n"] += 1
            return {
                "score": 5, "keywords": "", "reasoning": "ok",
                "matched_skills": [], "missing_requirements": [],
                "baseline_score": 5, "llm_score": 5, "llm_confidence": 0.5,
                "score_delta": 0, "normalized_title": "job",
            }

        monkeypatch.setattr(scorer, "get_connection", lambda: test_db)
        monkeypatch.setattr(scorer, "_load_scoring_resume_text", lambda: "resume")
        monkeypatch.setattr(scorer, "score_job", mock_score)
...
class TestTailoringE2EScoping:
    """Verify run_tailoring only tailors the target job when job_url is set."""

    def test_tailoring_scoped_to_single_url(self, test_db, monkeypatch, tmp_path):
        from applypilot.scoring import tailor

        _insert_job(test_db, "https://target.com/1", "Target SWE", site="acme",
                    desc="Python role", score=8)
        _insert_job(test_db, "https://other.com/2", "Other Job", site="corp",
                    desc="Sales role", score=9)

        tailored_dir = tmp_path / "tailored"
        tailored_dir.mkdir()

        monkeypatch.setattr(tailor, "get_connection", lambda: test_db)
        monkeypatch.setattr(tailor, "TAILORED_DIR", tailored_dir)
        monkeypatch.setattr(tailor, "load_resume_text", lambda: "Python dev resume")
        ...
        monkeypatch.setattr(tailor, "tailor_resume", lambda *a, **kw: (
            "Tailored resume text",
            {"attempts": 1, "validator": {"passed": True, "errors": [], "warnings": []},
             "judge": {"passed": True, "verdict": "PASS", "issues": "none", "raw": "VERDICT: PASS\nISSUES: none"},
             "status": "approved", "validation_mode": "normal"},
        ))

        # CHANGED: upstream renamed job_url to target_url in run_tailoring
        tailor.run_tailoring(min_score=7, target_url="https://target.com/1")

        target = test_db.execute("SELECT tailored_resume_path FROM jobs WHERE url = 'https://target.com/1'").fetchone()
        other = test_db.execute("SELECT tailored_resume_path FROM jobs WHERE url = 'https://other.com/2'").fetchone()
        assert target["tailored_resume_path"] is not None
        assert other["tailored_resume_path"] is None
...
class TestCoverLetterE2EScoping:
    ...
    def test_cover_scoped_to_single_url(self, test_db, monkeypatch, tmp_path):
        ...
        _insert_job(test_db, "https://target.com/1", "Target SWE", site="acme",
                    desc="Python role", score=8, tailored="/path/a.txt")
        _insert_job(test_db, "https://other.com/2", "Other Job", site="corp",
                    desc="Sales role", score=9)  # no tailored resume → not eligible

        cover_dir = tmp_path / "covers"
        cover_dir.mkdir()

        monkeypatch.setattr(cover_letter, "get_connection", lambda: test_db)
